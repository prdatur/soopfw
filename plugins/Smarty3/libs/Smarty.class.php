...
if (!defined('SMARTY_RESOURCE_DATE_FORMAT')) {
    define('SMARTY_RESOURCE_DATE_FORMAT', '%d.%m.%Y %H:%M:%S');
}
...
class Smarty extends Smarty_Internal_Data {
    ...
    public function set_tpl($template_dir)
    {
    	$this->template_dir = $template_dir;
    }

	public function get_tpl($dontReplaceSitepath = false)
    {
		if($dontReplaceSitepath == true)
		{
			return $this->template_dir;
		}
    	return preg_replace("/\/+$/", '', str_replace(SITEPATH, "", $this->template_dir));
    }
    ...
    public function fetch($template, $cache_id = null, $compile_id = null, $parent = null, $display = false)
    {
		if (!empty($this->tpl_vars))
		{
			...
			foreach($this->tpl_vars as $key => &$value) {
				if ($key == 'LANG' || $key ==  "JS_LANG") continue;
				if (substr($key, 0, 9) == 'UNESCAPED') continue;
				$this->make_safeVars_htmlspecialchars_recursive($value->value);
			}
		}
	...
	public function make_safeVars_htmlspecialchars_recursive(&$value)
	{
		...
		if (is_array($value))
		{
			foreach($value as $key => &$val) {
				...
				if (substr($key, 0, 9) == 'UNESCAPED') continue;
				$this->make_safeVars_htmlspecialchars_recursive($val);
			}
		}
