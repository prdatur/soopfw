...
class ParamStruct extends Object
{
	...
	public function add_validator($field, $validators) {
		...
		foreach ($validators AS $validator) {
			...
			$validator->set_value($this->__get($field));
		}
	}
	...
	public function add_required_param($name, $typ, $map_as = "") {
		...
		$this->add_validator($name, new RequiredValidator());
	}
	...
	public function __get($name) {
		...
		if (!isset($this->struct[$name]) || $this->is_empty($this->struct[$name]['value'], $this->get_param_type($name))) {
			...
			//If we have a parameter with that name return the value
				...
				//We have no such parameter, if on debug mode trigger an error and return null
				if ($this->core->get_debug() == true) {
					trigger_error(" Undefined Param $name");
				}
				return null;
			}
		}
