...
class Menu extends ActionModul
{
	 ...
	 * This saves some code lines because we always want to check for permissions within every request action.
	 *
	...
	public function entries($menu_id) {
		...
		// Load the menu object and verfiy that it exist.
		$menu_entry_obj = new MenuObj($menu_id);
		if (!$menu_entry_obj->load_success()) {
			throw new SoopfwWrongParameterException(t('No such menu'));
		}

		$this->title(t("\"@menu\" menu entries", array("@menu" => $menu_entry_obj->title)), t("Change the order of one or more menu entries and add manually menu entries"));
		...
		foreach ($this->db->query_slave_all("SELECT `entry_id`, `parent_id`, `order` FROM `" . MenuEntryObj::TABLE . "` WHERE menu_id = @menu_id", array('@menu_id' => $menu_id)) AS $menu_entry) {
			...
			// If we have setup an entry with the current language take this.
			if (isset($translations[$this->core->current_language])) {
				$menu_translation = $translations[$this->core->current_language];
			}
			// else take the first one within the translations.
			else {
